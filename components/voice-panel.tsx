  ...
  return (
    <div className="flex flex-col h-full p-4">
      <div className="h-[65vh] overflow-y-auto flex-col gap-4">
        ...
        {micPermission === "prompt" && (
          <div className="p-3 bg-yellow-100 text-yellow-700 rounded-md text-sm">
            <p>
              Microphone permission needed. Please click the voice button to
              enable.
            </p>
          </div>
        )}
            ...
            {messages.map((message) => {
              ...
              const parts =
                message.role === "assistant"
                  ? [message.parts.findLast((part) => part.type === "text")]
                  : message.parts;
              const toolParts = message.parts.filter((part) => part?.type === 'tool-deleteTask' || part?.type === 'tool-updateTask');
                  ...
                  <MessageContent>
                    ...
                    })}
                    {
                      toolParts.map((part, i) => {
                        if (!part) return null;
                        if (part.type === 'tool-deleteTask' || part.type === 'tool-updateTask') {
                          console.log("tool object :", part);
                          const toolName = getToolName(part);
                          const toolCallId = part.toolCallId;
                          if (part.state === "input-available") {
                            return (
                              <div key={`${part.toolCallId}-tool-${i}`}>
                                <p>Do you confirm this action : {toolName}?</p>
                                <div className="flex gap-2">
                                  ...
                                  onClick={async () => {
                                    ...
                                    sendMessage(undefined, { body: { tasks } });
                                    setTimeout(() => {
                                      invalidateTasks();
                                    }, 1500);
                                  }}
                                >
                                  ...
                                  onClick={async () => {
                                    ...
                                    await sendMessage(undefined, { body: { tasks } });
                                    setTimeout(() => {
                                      invalidateTasks();
                                    }, 1500);
                                  }}
      ...
      <p className="text-xs text-center mx-auto text-muted-foreground max-w-xs">
        Click the mic and speak. Speech is auto-detected and processed. Turning
        it off stops the detection.
      </p>
